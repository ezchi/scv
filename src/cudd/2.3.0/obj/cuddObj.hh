/**C++HeaderFile***************************************************************
  ...
  Revision    [$Id: cuddObj.hh,v 1.1.12.1 2003/05/01 20:55:09 dshea Exp $]
...
class DD {
    ...
    inline void checkReturnValue(const int result, const int error = 0);
    inline void checkReturnValue(
      const double result,
      const double error = CUDD_OUT_OF_MEM);
public:
    DD(Cudd *ddManager, DdNode *ddNode);
    DD();
    DD(const DD &from);
    Cudd *manager();
    inline DdNode * getNode(){return node;}
    int nodeCount();
...
extern "C" {
  typedef DdNode *CuddPiFuncT(
    DdManager * , int, DdNode **, DdNode **, DdNode **
  );
}
...
extern "C" {
  typedef int CuddHookFuncT(DdManager *, char *, void *);
}
...
public:
    ...
    void AddHook(CuddHookFuncT *f, Cudd_HookType where);
    void RemoveHook(CuddHookFuncT *f, Cudd_HookType where);
    int IsInHook(CuddHookFuncT *f, Cudd_HookType where);
...
EXTERN void defaultError(char *message);
